...
function loadmap(){
	...
	var mapProj = new ol.proj.configureProj4jsProjection({
	  code: 'EPSG:900913',
	  units: ol.proj.Units.METERS
	});
	var wgsProj = new ol.proj.configureProj4jsProjection({
	  code: 'EPSG:4326',
	  units: ol.proj.Units.METERS
	});
	var britProj = new ol.proj.configureProj4jsProjection({
	  code: 'EPSG:27700',
	  units: ol.proj.Units.METERS
	});
	
	//bounds = [-6.2400, 49.9400, 1.7800, 58.6700];
	//bounds = ol.projection.transform(bounds, 'EPSG:4326', 'EPSG:900913');
	...
	/*var insm = new ol.style.Style({rules: [
		new ol.style.Rule({
		  symbolizers: [
			new ol.style.Fill({
			  color: '#ffffff',
	...
	/*var selectFeatureControl = new ol.interaction.Select({
		layerFilter: function(geojson_layer) { return layer.get('id') == 'vector'; }
	});*/
	...
	/*var hosm = new ol.style.Style({rules: [
		new ol.style.Rule({
		  symbolizers: [
			new ol.style.Fill({
			  color: '#4169E1',
	...
	/*eventPoints = new ol.layer.Vector({
		style: hosm
	});
	
	//event stuff
	var selectFeatureControl2 = new ol.interaction.Select({
		layerFilter: function(eventPoints) { return layer.get('id') == 'vector'; }
	});*/
	...
	*/
	...
	//Define the map in OpenLayers 3 
	map = new ol.Map({
		//interactions: [ol.interaction.defaults().extend([selectFeatureControl]), ol.interaction.defaults().extend([selectFeatureControl2])],
		//layers: [osmLayer, geojson_layer, pointLayer, eventPoints],
		layers: [osmLayer],
		renderers: ol.RendererHints.createFromQueryData(),
		target: 'mapPanel',
		//size: [400, 300],
		view: new ol.View2D({
			//center: [ol.projection.transform(new ol.Coordinate(405000,285000), 'EPSG:27700', 'EPSG:900913')],
			projection: mapProj,
			zoom: 9
		})
	});
